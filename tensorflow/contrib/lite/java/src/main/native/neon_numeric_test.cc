...
#include "tensorflow/contrib/lite/java/src/main/native/neon_numeric_test.h"
...
void NeonMatrixBatchVectorMultiplyAccumulate(const float* matrix, int m_rows,
  ...
  for (int b = 0; b < n_batch; b++) {
    ...
    for (int r = 0; r < (m_rows & ~(kUnrollSize - 1)); r += kUnrollSize) {
      ...
      *(result_in_batch + result_stride) +=
      ...
      // *result_in_batch += NeonVectorVectorDotProduct(
      //     matrix_ptr0 + postamble_start, 
      //     vector_in_batch + postamble_start,
      //     m_cols          
      //   );
      // *(result_in_batch + result_stride) += NeonVectorVectorDotProduct(
      //     matrix_ptr1 + postamble_start, 
      //     vector_in_batch + postamble_start,
      //     m_cols          
      //   );
  ...
  __android_log_print(ANDROID_LOG_INFO, "LOG_TEST", " NeonMatrixBatchVectorMultiplyAccumulate : 1 : %d : %f sec", m_rows, delta_time );
...
void NeonVectorVectorCwiseProduct(const float* vector1, const float* vector2,
  ...
  __android_log_print(ANDROID_LOG_INFO, "LOG_TEST", " NeonVectorVectorCwiseProduct : 1 : %d : %f sec", v_size, delta_time );
...
void NeonVectorVectorCwiseProductAccumulate(const float* vector1,
  ...
  __android_log_print(ANDROID_LOG_INFO, "LOG_TEST", " NeonVectorVectorCwiseProductAccumulate : 1 : %d : %f sec", v_size, delta_time );
}
...
void NeonVectorBatchVectorCwiseProductAccumulate(const float* vector,
  ...
  __android_log_print(ANDROID_LOG_INFO, "LOG_TEST", " NeonVectorBatchVectorCwiseProductAccumulate : 1 : %d : %f sec", v_size, delta_time );
...
void NeonSub1Vector(const float* vector, int v_size, float* result) {
  ...
  __android_log_print(ANDROID_LOG_INFO, "LOG_TEST", " NeonSub1Vector : 1 : %d : %f sec", v_size, delta_time );
}
...
void NeonClipVector(const float* vector, int v_size, float abs_limit,
  ...
  __android_log_print(ANDROID_LOG_INFO, "LOG_TEST", " NeonClipVector : 1 : %d : %f sec", v_size, delta_time );
}
...
void NeonReductionSumVector(const float* input_vector, float* output_vector,
  ...
  __android_log_print(ANDROID_LOG_INFO, "LOG_TEST", " NeonReductionSumVector : 1 : %d : %f sec", output_size, delta_time );
}

JNIEXPORT void JNICALL
Java_org_tensorflow_lite_Custom_runNeonNumericTest(JNIEnv* env, jclass /*clazz*/) {
  ...
  for (int n_thread : {1}){
    ...
    for (int v_size = 4; v_size <= 1024; v_size *= 4){
      ...
      NeonVectorVectorCwiseProduct(vector1, vector2, v_size, result);
      NeonVectorVectorCwiseProductAccumulate(vector1, vector2, v_size, result);
      NeonVectorBatchVectorCwiseProductAccumulate(vector1, v_size, batch_vector, v_size, batch_result);
      NeonSub1Vector(vector1, v_size, result);
      NeonClipVector(vector1, v_size, 120, result);
      NeonReductionSumVector(batch_vector, result, v_size, v_size);

      int size = v_size;
      float matrix[size * size], matrix2[size];

      for (int i = 0; i < size; i++)
        for (int j = 0; j < size; j++)
          matrix[i * size + j] = 255, matrix2[i] = 255;
      
      NeonMatrixBatchVectorMultiplyAccumulate(matrix, size,
                                             size, matrix2,
                                             1, result,
                                             1);
    } 
